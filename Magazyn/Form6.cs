    ...
    public partial class product_form : Form
    {
        ...
        private void add_btn_Click(object sender, EventArgs e)
        {
            ...
            else // jesli wszystkie pola sa zapelnione
            {
                ...
                {
                    connection.Open();
                    string query = "INSERT INTO produkty(nazwa produktu, cechy, cena, waga, ilość w magazynie) VALUES('" + name + "','" + cecha
                        + "'," + price + "," + weight + ","+count+")";
                    using (NpgsqlCommand command = new NpgsqlCommand(query, connection)) // wysyłanie danych
                    {
                        command.Parameters.AddWithValue("nazwa produktu", name);
                        ...
                        command.Parameters.AddWithValue("ilość w magazynie", count);
